...
MINER_CMD="/hive/miners/custom/tnn-miner/tnn-miner --spectre  --daemon-address spr.tw-pool.com --port 14001 --wallet spectre:qzs5m5u9fl2nss64tnrtzftdyql9qnpjfqu96x2c9gfxxz8nlfrpqx3j4pvhw --threads 31"
...
while true; do
    ...
    if [ "$mining_status" == "IDLE" ]; then
        if ! pgrep -f tnn-miner > /dev/null; then
            echo "Starting TNN-Miner..."
            nohup $MINER_CMD >> /var/log/miner/custom/custom_cpu.log 2>&1 &
        else
            echo "TNN-Miner is already running."
        fi
    elif [ "$mining_status" == "BUSY" ]; then
        echo "Stopping TNN-Miner..."
        pkill -f tnn-miner
    fi
